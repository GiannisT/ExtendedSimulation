...
public class ServiceProviderManager extends TimerTask {
    ...
    public void start()
    {
        synchronized (RUNNING_LOCK)
        {
            if(!this.running)
            {
                this.counter=Utilities.getBundle("serviceProvider.max", 4); //how many SP will be created, if changed, the serviceProvider.max attribute in init.properties should change as well
    ...
    public synchronized void run()
    {
        ...
        if (isRunning() && counter >0)
        {
          ...
          //************************************************************ END GIANNIS CODE********************************************************************
            ...
            AskCreator creator=new AskCreator(serviceProvider, 100, isRandom);
            creator.start();
        }
        counter--;
        if(counter<=0)
